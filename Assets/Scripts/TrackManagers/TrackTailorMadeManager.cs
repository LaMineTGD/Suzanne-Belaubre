...
public class TrackTailorMadeManager : ITrackManager
{
    ...
    protected int base_rate_value = 0;
    ...
    protected override void ApplyDefaultEffects() {
        ...
        if (m_VFX!= null) {
            base_rate_value = m_VFX.GetInt(rate_name);
            m_VFX.SetInt(rate_name,0);
        }
    ...
    protected void VFXTransition(bool isVisible, float duration){
        if (m_VFX!= null) 
            StartCoroutine(Utils.Utils.InterpolatVfxIntVisibility(isVisible,rate_name,base_rate_value,m_VFX,duration));
    }
}
